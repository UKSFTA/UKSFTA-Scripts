/*
 * Author: UKSFTA Team
 * Description: Simplified ATragMX preset generator from current weapon.
 *
 * Parameter:
 * 0: OBJECT - The unit to generate for.
 *
 * Return:
 * NOTHING
 */
...
private _weapon = (_weaponsInfo select _selectedWeaponInfo) select 0;
private _magazine = (_weaponsInfo select _selectedWeaponInfo) select 4;
...
private _muzzleVel_avg = [getArray(_ammoConfig >> "ACE_muzzleVelocities")] call _getAverage;
...
private _presetName = format ["%1 (%2) [Quick]", _weaponDisplayName, _ammoClassName];
...
hint format ["Preset generated for %1", _presetName];
